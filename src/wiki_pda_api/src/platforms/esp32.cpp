...
static bool esp32_sd_read(uint64_t offset, uint8_t* buf, uint32_t len, void* user_data) {
    Esp32SdInternalContext* ctx = (Esp32SdInternalContext*)user_data;
...
DatabasePlatform platform_esp32(void* file_handle) {
    ...
    Esp32SdInternalContext* ctx = new Esp32SdInternalContext();
    ctx->card = card;
    ...
    platform.read_fn = esp32_sd_read;
