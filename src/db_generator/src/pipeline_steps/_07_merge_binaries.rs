...
pub fn merge_into_master_database(settings: &Settings) -> Result<(), String> {
    ...
    // 5. Update info.json with the generated byte offsets and sizes
    ...
    let mut sizes_json_obj = serde_json::Map::new(); // New sizes map

    let mut omni_search_level_offsets = serde_json::Map::new();
    let mut omni_search_level_sizes = serde_json::Map::new(); // New nested sizes map

    // Destructure the tuple (offset, size) here
    for (key, (offset, size)) in file_info {
        if key.starts_with("omni_search_level_") {
            ...
            // Keep qid_hashmap, qid_index, etc., at the root
    ...
    // Insert the nested omni_search_level objects into the main objects
    ...
    // Insert BOTH main objects into the root info.json object
    root_obj.insert(
        "offsets".to_string(),
...
fn merge_files(
    ...
    // 1. Changed return type to tuple (offset, size)
    let output_file = File::create(output_combined_path).map_err(|e| e.to_string())?;
    let mut writer = BufWriter::new(output_file);

    // Renamed variable to reflect it now holds multiple pieces of info
    ...
    for item in files_to_merge {
        ...
        // 2. Insert BOTH the current offset and the file size into the map
