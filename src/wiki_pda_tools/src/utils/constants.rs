// If you change this, you need to change
// MAX_SPARSE_LEVELS in 'wiki_pda_api/src/common/common.h'
// as well.
pub const SD_CARD_SECTOR_SIZE: u64 = 512;
pub const HEADER_SIZE_BYTES: u64 = 4096;
pub const MAX_SPARSE_LEVELS: usize = 10;
pub const DELETE_SOURCE_BINARIES_AFTER_MERGE: bool = true;

// Files
// Temporary files
pub const OMNI_SEARCH_TXT: &str = "omni_search.txt";
pub const PROPERTIES_SEARCH_TXT: &str = "properties_search.txt";
pub const SITELINKS_QID_MAPPING_DB: &str = "sitelinks_qid_mapping.db";
pub const QID_INDEX_TXT: &str = "qid_index.txt";
pub const META_DATA_TXT: &str = "meta_data.txt";
pub const GLOBE_COORDINATE_SEARCH_TXT: &str = "globe_coordinate_search.txt";
pub const ASTRONOMICAL_SEARCH_TXT: &str = "astronomical_search.txt";
pub const TEMPORAL_SEARCH_TXT: &str = "temporal_search.txt";
pub const WIKI_LANG_MAPPING_TXT: &str = "wiki_lang_mapping.txt";
pub const INFO_JSON: &str = "info.json";

// Binary files
pub const DATA_BASE_BIN: &str = "data_base.bin";
pub const ZSTD_DICTIONARY_BIN: &str = "zstd_dictionary.bin";
pub const CONTENT_BIN: &str = "content.bin";
pub const QID_HASHMAP_BIN: &str = "qid_hashmap.bin";
pub const QID_INDEX_BIN: &str = "qid_index.bin";
pub const TITLES_BIN: &str = "titles.bin";
pub const PID_HASHMAP_BIN: &str = "pid_hashmap.bin";
pub const PID_INDEX_BIN: &str = "pid_index.bin";
pub const PID_STRINGS_BIN: &str = "pid_strings.bin";
pub const META_DATA_BIN: &str = "meta_data.bin";
pub const OMNI_SEARCH_BIN: &str = "omni_search.bin";
pub const TEMPORAL_SEARCH_BIN: &str = "temporal_search.bin";
pub const ASTRONOMICAL_SEARCH_BIN: &str = "astronomical_search.bin";
pub const GLOBE_COORDINATE_SEARCH_BIN: &str = "globe_coordinate_search.bin";

pub const OMNI_SEARCH_SPARSE_INDEX_TEMPLATE_BIN: &str = "omni_search_sparse_index.bin";
pub const TEMPORAL_SEARCH_SPARSE_INDEX_TEMPLATE_BIN: &str = "temporal_search_sparse_index.bin";
pub const GLOBE_COORDINATE_SEARCH_SPARSE_INDEX_TEMPLATE_BIN: &str =
    "globe_coordinate_search_sparse_index.bin";
pub const ASTRONOMICAL_SEARCH_SPARSE_INDEX_TEMPLATE_BIN: &str =
    "astronomical_search_sparse_index.bin";

// Log files
pub const DOWNLOAD_LOG: &str = "download.log";
pub const PARSER_LOG: &str = "parser.log";
pub const COMPRESSION_SETUP_LOG: &str = "compression_setup.log";
pub const ZIM_PROCESSING_LOG: &str = "zim_proccesing.log";
pub const MAKE_METADATA_BINARY_LOG: &str = "create_metadata_binary.log";
pub const MAKE_QID_BINARY_INDEX_LOG: &str = "create_qid_binary_index.log";
pub const MAKE_PID_BINARY_INDEX_LOG: &str = "create_pid_binary_index.log";
