...
def download_wikidata_dump():
    ...
    url = cfg.get("WIKIDATA_URL")
    if not url:
        print("Error: WIKIDATA_URL is missing in downloader.config!")
        return

    filename = url.split("/")[-1]

    wikidata_dir_rel = os.path.normpath(cfg.get("WIKIDATA_PATH", "zim_files"))
    wikidata_dir_abs = os.path.join(repo_root, wikidata_dir_rel)

    destination_path = os.path.join(wikidata_dir_abs, filename)

    print(f"Starting download of Wikidata Sitelinks...")
    print(f"Source:      {url}")
    print(f"Destination: {destination_path}")

    os.makedirs(wikidata_dir_abs, exist_ok=True)
