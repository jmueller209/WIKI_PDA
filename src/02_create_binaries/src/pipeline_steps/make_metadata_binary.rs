...
// 1. Import the progress bar tools
...
pub fn make_metadata_binary(settings: &Settings) -> Result<(), String> {
    ...
    // 2. Get the total file size in bytes directly from the OS metadata
    ...
    // 3. Initialize the progress bar with a clean visual template
    ...
    for line_result in reader.lines() {
        ...
        // 4. Advance the progress bar by the byte length of the string + 1 (to account for the newline character `\n` that .lines() strips off)
        ...
        if let Some((qid, content)) = line.split_once(txt_delimiter) {
            ...
            // pb.println() prints above the progress bar without breaking it graphically!
            pb.println(format!(
    ...
    // 5. Clean up the progress bar from the terminal once finished
    pb.finish_and_clear();
